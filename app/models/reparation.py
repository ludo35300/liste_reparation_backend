...
class Reparation(db.Model):
    ...
    # ── Snapshot texte (conservé pour historique et rétrocompat) ──
    machine_type    = db.Column(db.String(200), nullable=False)
    technicien      = db.Column(db.String(100), default='')

    # ── FK relationnelles (nullable → migration additive sans casser l'existant) ──
    ...
    machine_ref     = db.relationship('MachineTypeRef', foreign_keys=[machine_type_id], lazy='select')
